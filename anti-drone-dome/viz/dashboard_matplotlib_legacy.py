...
DEPRECATED: superseded by the PyQtGraph dashboard in ``viz.dashboard``. This file
is kept as a historical reference. Import is gated to prevent accidental use;
set the env var ``DOME_USE_LEGACY_MPL_DASHBOARD=1`` to bypass the guard.
"""

import os as _os

if not _os.environ.get("DOME_USE_LEGACY_MPL_DASHBOARD"):
    raise ImportError(
        "viz.dashboard_matplotlib_legacy is deprecated. "
        "Use 'from viz.dashboard import Dashboard, SimControl' (PyQtGraph version) instead. "
        "Set DOME_USE_LEGACY_MPL_DASHBOARD=1 to force-load the legacy file."
    )
