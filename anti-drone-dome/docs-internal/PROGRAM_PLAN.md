...
- `sensors/radar_model.py` — **DONE.** The radar range equation plus Shnidman's P<sub>d</sub>(SNR, P<sub>fa</sub>, N, Swerling) approximation. ~280 lines, no licence, runs in CI. Replaces the hand-drawn piecewise curve formerly at `sensors/radar.py:360-372`, which had no radar equation behind it: no transmit power, no antenna gain, no wavelength, no noise figure, no integration gain, and a `false_alarm_probability` that was a free scenario parameter rather than derived from a detection threshold — **so P<sub>d</sub> and P<sub>fa</sub> were decoupled, which is physically impossible.**

  Validated in `tests/test_radar_model.py` against published reference values: non-fluctuating single-pulse P<sub>d</sub>=0.9 at P<sub>fa</sub>=1e-6 needs 13.12 dB (textbook ~13.2); P<sub>d</sub>=0.5 needs 11.17 dB (~11.2); Swerling-1 P<sub>d</sub>=0.9 needs 21.35 dB (~21.1). The R<sup>-4</sup> law and linear-in-RCS behaviour are pinned exactly, and R ∝ σ<sup>1/4</sup> is verified. **That agreement is the credibility artifact a MATLAB toolbox would otherwise have bought.**

  **The measured delta, published at `reports/detection/detection_model_comparison.csv`, is not what we expected and should be reported as such:** the physics model is *more* optimistic than the legacy curve nearly everywhere — +0.41 at 1000 m for a 0.05 m² target, +0.63 at 600 m for 0.01 m². The old curve was pessimistic in a way grounded in nothing. Two consequences: (a) intercept rates will rise slightly when this lands, which works *against* the goal of producing honest failures in §2 — the P<sub>k</sub> failures must come from real sensing and association (S1), not from an arbitrarily harsh detection curve; (b) the model now responds correctly to RCS and to every term of the link budget, so degrading the budget (rain, a cheaper front end, jamming) produces physically correct degradation, which the fitted curve structurally could not do. Regenerate with `scripts/compare_detection_models.py`.
