...
def main() -> int:
    ...
    status = "pass" if device_present else ("partial" if cli_available else "fail")
    record = {"schema": SCHEMA, **metadata(root, {"operation": "hailo_bringup"}),
              "measurement_status": status, "operator": args.operator,
              "device_present": device_present, "commands": {"identify": identify, "scan": scan},
              "versions": versions,
