...
Updates reuse a single polar ``scatter`` artist (``set_offsets``) instead of
``ax.clear()`` every frame, which was very slow and could destabilize the GUI.
"""
...
class RadarHUD:
    ...
    def __init__(
        ...
        self._fig_closed_cid = self._fig.canvas.mpl_connect("close_event", self._on_figure_closed)
        ...
        theta0 = np.zeros(num_drones)
        r0 = np.zeros(num_drones)
        facecolors = [self._cmap(i % 10) for i in range(num_drones)]
    ...
    def _on_figure_closed(self, event) -> None:
        """User closed the Matplotlib window — end the sim cleanly."""
        self.stop_requested = True
    ...
    def update(self, positions: np.ndarray) -> None:
        ...
        self._sc.set_offsets(np.column_stack((theta, r)))
        lines = [
    ...
    def close(self) -> None:
        if self._fig is not None:
            try:
                self._fig.canvas.mpl_disconnect(self._fig_closed_cid)
