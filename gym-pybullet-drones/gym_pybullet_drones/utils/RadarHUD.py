...
(no OpenGL overlay hooks). The radar axes are placed in the bottom-left of
that figure; numeric XYZ for each drone is listed underneath.
"""
...
class RadarHUD:
    """Top-down (X–Y) polar view with per-drone world-frame XYZ text."""
    ...
    def __init__(
        ...
        # Bottom-left polar radar (figure coordinates: left, bottom, width, height)
        self._ax = self._fig.add_axes([0.12, 0.30, 0.76, 0.62], projection="polar")
        ...
        # Coordinate block under radar, bottom-left aligned
